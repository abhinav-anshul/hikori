-- =============================================================
-- get_analytics(p_from, p_to, p_bucket, p_link_id, p_exclude_bots) — v5
--   • Adds top_browsers and top_os aggregations (null buckets dropped),
--     mirroring the existing top_devices pattern.
--
-- Output shape:
--   {
--     "total": int,
...
--     "top_browsers":  [{ "browser", "clicks" }, ...]
--     "top_os":        [{ "os",      "clicks" }, ...]
--   }
-- =============================================================
drop function if exists public.get_analytics(timestamptz, timestamptz, text, uuid, boolean);
...
begin
  ...
  -- top browsers (drop nulls)
  select jsonb_agg(
           jsonb_build_object('browser', browser_name, 'clicks', clicks)
           order by clicks desc
         )
    into v_top_browsers
    from (
  ...
  -- top OS (drop nulls)
  select jsonb_agg(
           jsonb_build_object('os', os_name, 'clicks', clicks)
           order by clicks desc
         )
    into v_top_os
    from (
      select os_name, count(*) as clicks
        from public.click_events
       where user_id = v_user_id
         and created_at >= v_from
         and created_at <  v_to
  ...
  return jsonb_build_object(
    ...
    'top_browsers',  coalesce(v_top_browsers,  '[]'::jsonb),
    'top_os',        coalesce(v_top_os,        '[]'::jsonb)
  );
end $$;
